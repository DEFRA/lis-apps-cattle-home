...
// A keeper with none has nothing to land on. Holdings come from the hub
// JWT (resolved from identity-service-helper's directAssignments at login),
// the same source KRDS/cattle-api holding details are keyed on - not a
// spoke-local lookup.
export const landingController = {
  handler(request, h) {
    ...
    return h.redirect(
      `${holdingDetailsBasePath}/${cphPath(holdings[0].countyParishHoldingNumber)}`
    )
  }
}
